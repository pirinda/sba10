 ...
 */
public class DUtilConfigApp extends JFrame {

    /** Creates new form DUtilConfigApp */
    public DUtilConfigApp() {
        initComponents();
    ...
    private void readConfigXml() {
        String xml = "";
        String timeZone = "";
        DUtilConfigXml cfgXml = null;

        try {
            xml = DXmlUtils.readXml(DUtilConsts.FILE_NAME_CFG);
            cfgXml = new DUtilConfigXml();
            cfgXml.processXml(xml);
            
            jtfDbHost.setText(cfgXml.getAttribute(DUtilConfigXml.ATT_DB_HOST).getValue().toString());
            jtfDbPort.setText(cfgXml.getAttribute(DUtilConfigXml.ATT_DB_PORT).getValue().toString());
            jtfDbName.setText(cfgXml.getAttribute(DUtilConfigXml.ATT_DB_NAME).getValue().toString());
            jtfUserName.setText(cfgXml.getAttribute(DUtilConfigXml.ATT_USR_NAME).getValue().toString());
            jpfUserPassword.setText(cfgXml.getAttribute(DUtilConfigXml.ATT_USR_PSWD).getValue().toString());
            jtfTerminal.setText("" + cfgXml.getAttribute(DUtilConfigXml.ATT_TERMINAL).getValue());

            jcbTimeZone.setSelectedIndex(0);
            timeZone = cfgXml.getAttribute(DUtilConfigXml.ATT_TIME_ZONE).getValue().toString();
    ...
    private boolean saveConfigXml() {
        ...
        try {
            cfgXml = new DUtilConfigXml();
            cfgXml.getAttribute(DUtilConfigXml.ATT_DB_HOST).setValue(DLibUtils.textTrim(jtfDbHost.getText()));
            cfgXml.getAttribute(DUtilConfigXml.ATT_DB_PORT).setValue(DLibUtils.textTrim(jtfDbPort.getText()));
            cfgXml.getAttribute(DUtilConfigXml.ATT_DB_NAME).setValue(DLibUtils.textTrim(jtfDbName.getText()));
            cfgXml.getAttribute(DUtilConfigXml.ATT_USR_NAME).setValue(DLibUtils.textTrim(jtfUserName.getText()));
            cfgXml.getAttribute(DUtilConfigXml.ATT_USR_PSWD).setValue(DLibUtils.textTrim(new String(jpfUserPassword.getPassword())));
            cfgXml.getAttribute(DUtilConfigXml.ATT_TERMINAL).setValue(DLibUtils.parseInt(jtfTerminal.getText()));
            cfgXml.getAttribute(DUtilConfigXml.ATT_TIME_ZONE).setValue(DLibUtils.textTrim(jcbTimeZone.getSelectedItem().toString()));

            DXmlUtils.writeXml(cfgXml.getXmlString(), DUtilConsts.FILE_NAME_CFG);
    ...
    */
    public static void main(String args[]) {
        try {
            ...
            DLibUtils.showException(DUtilConfigApp.class.getName(), e);
        }

        java.awt.EventQueue.invokeLater(new Runnable() {
            @Override
            public void run() {
                new DUtilConfigApp().setVisible(true);
            }
        });
    }
